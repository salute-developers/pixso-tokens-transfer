...
import { PopupProvider, Button } from '@salutejs/sdds-serv';
...
const App = () => {
    ...
    const [flow, setFlow] = useState<FlowType>('export');
    ...
    const startExport = () => {
        ...
        const handleStylesForExport = (message: { data: RemoteStyleData[] }) => {
            pixsoEventBus.off(CONSTANTS.msgType.localStylesForExport, handleStylesForExport);
            addLog(`Собрано ${message.data.length} стилей. Формирование файла...`);

            try {
                const result = {
                    id: selectedSystem!.id,
                    name: selectedSystem!.name,
                    projectName: selectedSystem!.projectName,
                    localStyles: message.data,
                };

                const timestamp = Date.now();
                const filename = `${selectedSystem!.name}_${timestamp}.json`;
                ...
                addLog(`Файл "${filename}" сформирован`);
                setResultState('success');
            } catch (error) {
                addLog(`Ошибка: ${error instanceof Error ? error.message : 'неизвестная ошибка'}`);
                setResultState('error');
        ...
        switch (s) {
            ...
            case 'confirm':
                return selectedSystem ? (
                    <ConfirmScreen flow={flow} system={selectedSystem} onConfirm={handleConfirm} />
                ) : null;
            case 'result':
